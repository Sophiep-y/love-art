...
const ArtDetailSidebarStateLess = ({
  ...
  const sidebarClasses = `${isUnCollapse ? "block" : "hidden"}  ${sidebarBaseClasses} py-5 ${
    isNavigation ? "w-full" : "  md:w-1/3 w-full"
  } h-full top-0 bottom-0 pt-0.5 pl-5 ${isNavigation ? "left-0" : "right-0"} ${transitionClasses} ${isCollapsed ? "translate-x-full" : isNavigation ? "translate-x-0" : "-translate-x-0"}`;
  ...
  return (
    <div>
      ...
      <div className={`${sidebarClasses} overflow-y-scroll `}>
        ...
        <div>
          {/*artist*/}
          <div className="text-2xl">{data?.artist}</div>
          {/*title*/}
          <div className="text-2xl italic ">{data?.title}</div>
          {/*image*/}
          <div className="shadow-recommendation mr-8 my-4">
            <ImageWithLoading src={data?.imageUrl} alt="Card Image" />
          </div>
            ...
            Object.keys(info).map((key, index) => {
              ...
              return (
                <div key={index} className="mr-8 ">
                  <div
                    className={`flex items-start justify-items-start  ${item.needMargin ? "my-8" : ""}`}
                  >
