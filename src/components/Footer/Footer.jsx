...
export const Footer = ({ footer, absolute, sticky = true, ref, className }) => {
  ...
  return (
    <div
      className={`${isMobile ? "" : `flex bottom-0 ${sticky ? "fixed" : ""}`}`}
    >
      ...
      <footer
        ...
        className={`${className} ${
          absolute
            ...
            : sticky
              ? `md:flex  block justify-between bottom-0 left-4 sticky bg-white mb-4`
      ...
      >
        ...
        <div className="w-1/5 text-primary md:block hidden">
          ...
          <div className="mt-5">Site by Patrick Mason</div>
        </div>
      </footer>
      {footer && isMobile && (
        <div className="m-4 text-primary">Site by Patrick Mason</div>
      )}
    </div>
  );
};
